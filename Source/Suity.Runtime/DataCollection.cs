...
namespace Suity
{
    ...
    public class DataCollection : RuntimeResourceObject
    {
            ...
            foreach (var dataReader in reader.Nodes("Data"))
            {
                try
                {
                    ...
                    if (string.IsNullOrEmpty(key))
                    {
                        ...
                        {
                            Logs.LogError($"Data component Key is missing : {localId} in collection : {TableId}");
                        }
                        continue;
                    }
                    ...
                    foreach (var compReader in dataReader.Nodes("Component"))
                    {
                        try
                        {
                            ...
                            else
                            {
                                if (DataStorage.LogDataLoading)
                                {
                                    Logs.LogError($"Data component read failed : {compTypeName}, in collection : {TableId}");
                                }
                            }
                        }
                        catch (Exception e)
                        {
                            if (DataStorage.LogDataLoading)
                            {
                                Logs.LogWarning($"Data component read failed in collection : {TableId}");
                            }
